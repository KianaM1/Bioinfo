...
## Changes Added to Assignment 8
- Include in the README.md how to process a single sample (define variables)
- Include how to make the design.csv file
- Should figure out the warning but I don't know how
...
Following the example provided in the handbook, I made a design.csv file for the eight SRR numbers from the paper. I used the following command to make the design.csv file:
```
touch design.csv
```
I then typed in the information for each sample in the design.csv file in Visual Studio Code. 
...
I reused the Makefile from last week's assignment, but I added the commands to make a BigWig file to the "all" command. This ensures BigWig files are made when running the GNU parallel.

To run one sample using the Makefile, use the following command:
```
make all SRR=SRR3191542 SAMPLE=Sample1 GROUP=Control CONDITION=Mock1_1
```
